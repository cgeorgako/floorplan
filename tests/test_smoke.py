...
def test_hall_width_max_130():
    # Ο χωλ δεν πρέπει να έχει πλάτος μεγαλύτερο από 1,30 μ.
    for p in generate_proposals(_roomy(has_hall=True, num_proposals=6)):
        for fl in p.floors:
            for r in fl.rooms:
                if r.category == Category.HALL:
                    assert r.w <= 1.30 + 0.02, f"{r.name} πλάτος {r.w:.2f} > 1,30"


def test_every_room_has_access():
    # Κάθε χώρος (πλην κλίμακας) πρέπει να έχει τουλάχιστον ένα άνοιγμα
    # (θύρα/άνοιγμα/παράθυρο) → επικοινωνία/προσβασιμότητα.
    for shape in ("auto", "L", "T", "rectangular"):
        for p in generate_proposals(_spec(footprint_shape=shape, num_proposals=3)):
            for fl in p.floors:
                for r in fl.rooms:
                    if r.category == Category.STAIRS:
                        continue
                    assert r.openings, f"{shape}/{r.name}: χώρος χωρίς επικοινωνία"


def test_openings_clear_of_corners():
    # Κανένα άνοιγμα δεν πρέπει να «πέφτει» πάνω σε κάθετο τοίχο: αφήνει
    # ελεύθερη απόσταση και από τα δύο άκρα της πλευράς.
    for p in generate_proposals(_spec(num_proposals=3)):
        for fl in p.floors:
            for r in fl.rooms:
                for op in r.openings:
                    seg = r.w if op.side in ("N", "S") else r.d
                    assert op.offset >= 0.10 - 1e-6, f"{r.name}:{op.side} στη γωνία"
                    assert op.offset + op.width <= seg - 0.10 + 1e-6, \
                        f"{r.name}:{op.side} στη γωνία"
